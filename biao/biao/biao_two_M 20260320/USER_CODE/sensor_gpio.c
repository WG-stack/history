...
static void Process_SensorData(uint32_t raw_data)
{
    ...
    uint8_t nib0 = (raw_data >> 0)  & 0x0F;
    uint8_t nib1 = (raw_data >> 4)  & 0x0F;
    uint8_t nib2 = (raw_data >> 8)  & 0x0F;
    uint8_t nib3 = (raw_data >> 12) & 0x0F;
    uint8_t nib4 = (raw_data >> 16) & 0x0F;
    uint8_t nib5 = (raw_data >> 20) & 0x0F;
    uint32_t value_data = 0;
    value_data  = nib0;
    value_data |= ((uint32_t)nib1 << 4);
    value_data |= ((uint32_t)nib2 << 8);
    value_data |= ((uint32_t)nib3 << 12);
    value_data |= ((uint32_t)nib4 << 16);
    sensor_data.raw_data    = raw_data;
    sensor_data.is_negative = nib5 & 0x01;
    sensor_data.is_inch     = (nib5 >> 3) & 0x01;
    // 只做原始值存储，不再做单位换算（统一由 ConvertSensorValue 处理）
    // sensor_data.value 存储原始计数值（无符号）
    sensor_data.value       = (float)value_data;  // 单位由 measurement.c 决定
    sensor_data.data_valid  = 1;
    sensor_data.timestamp   = HAL_GetTick();
}
