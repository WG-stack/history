...
int main(void)
{
  ...
  // 禁用STOP模式和SLEEP模式，防止系统自动休眠
  // 在调试时保持系统运行
  HAL_DBGMCU_EnableDBGStopMode();
  HAL_DBGMCU_EnableDBGSleepMode();
  
  // 禁用Cortex-M3的SLEEPONEXIT和SLEEPDEEP位
  SCB->SCR &= ~(SCB_SCR_SLEEPONEXIT_Msk | SCB_SCR_SLEEPDEEP_Msk);
  
  // 应用初始化
  App_Init();
  
  // 启动TIM7中断，驱动按键扫描（50ms周期）
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin == GPIO_PIN_12)  // PC12下降沿触发首次采样
  {
    ...
    if (READ_DATA_BIT() == 1)  // 检查PC11数据位是否为1（数据头校验）
    {
      // 数据头有效，开始采样
      Sensor_GPIO_Read_24bit_Data();
      // 中断重使能在主循环内完成
    }
