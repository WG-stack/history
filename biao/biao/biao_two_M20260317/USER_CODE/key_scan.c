...
/**
  * @brief  检查是否同时按下了TOL + M + ON/ZERO键（用于进入工程模式）
  * @retval 1-是, 0-否
  */
uint8_t Key_IsEngineeringModePressed(void)
{
    uint8_t pressed = 0;
    
    // 如果TOL(PC13-PC14), M(PC13-PC15), ON/ZERO(PC14-PC15)同时按下，
    // 则PC13, PC14, PC15三根线会短接在一起。
    // 我们将PC13置低，PC14/15上拉输入，如果PC14和PC15都读到低电平，说明这三个键（或其中某几个组合导致全短路）被按下
    ...
    if ((LL_GPIO_IsInputPinSet(GPIOC, LL_GPIO_PIN_14) == 0) && 
        (LL_GPIO_IsInputPinSet(GPIOC, LL_GPIO_PIN_15) == 0)) {
        pressed = 1;
    }
