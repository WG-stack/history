...
typedef enum {
    TOL_SET_NONE = 0,
    TOL_SET_STANDARD, // 1. 设标准值：OK闪动
    TOL_SET_LOWER,    // 2. 设负公差：>|闪动
    TOL_SET_UPPER,    // 3. 设正公差：|<闪动
    TOL_SET_DONE      // 4. 设置完成：>|OK|< 常显2秒
} TolSetState_t;
// 定义相关静态变量
static TolSetState_t tol_set_state = TOL_SET_NONE;
static float temp_tol_standard = 0.0f;
static float temp_tol_lower = 0.0f;
static float temp_tol_upper = 0.0f;
static uint32_t tol_done_timer = 0;
/* Private variables ---------------------------------------------------------*/
...
//                LCD_Display_Update();
        ...
        case STATE_TOL_SETTING:
            // 公差设置
            // TODO: 实现公差设置逻辑
            // 1. 保持传感器数据读取，以便在设置标准值时能看到实时数据
            if (Sensor_GPIO_GetData(&sensor_data)) {
                Measure_Process(&sensor_data);
            }
            Sensor_GPIO_EnableInterrupt();
            if (has_key_msg) {
                App_ProcessKey(&last_key_msg); // 这里会调用我们上面修改的按键逻辑
            }
            // 2. 闪烁标志 (500ms 亮, 500ms 灭)
            uint8_t flash_flag = (HAL_GetTick() / 500) % 2;
            
            // 获取当前小数位数用于显示
            uint8_t dec_places = (Measure_GetData()->resolution == RESOLUTION_0_001MM) ? 3 : 2;
            LCD_Display_Clear();
            LCD_Display_Symbol(LCD_SYMBOL_TOL, 1); // TOL 始终常显
            if (tol_set_state == TOL_SET_STANDARD) {
                // 显示实时数据
                LCD_Display_Number(Measure_GetData()->current_value, dec_places);
                LCD_Display_Symbol(LCD_SYMBOL_OK, flash_flag); // OK闪动
                
            } else if (tol_set_state == TOL_SET_LOWER) {
                // 显示负公差
                LCD_Display_Number(temp_tol_lower, dec_places);
                LCD_Display_Symbol(LCD_SYMBOL_OK, 1);          // OK常显
                LCD_Display_Symbol(LCD_SYMBOL_TOL_LOWER, flash_flag); // >| 闪动
                
            } else if (tol_set_state == TOL_SET_UPPER) {
                // 显示正公差
                LCD_Display_Number(temp_tol_upper, dec_places);
                LCD_Display_Symbol(LCD_SYMBOL_OK, 1);          // OK常显
                LCD_Display_Symbol(LCD_SYMBOL_TOL_LOWER, 1);   // >| 常显
                LCD_Display_Symbol(LCD_SYMBOL_TOL_UPPER, flash_flag); // |< 闪动
                
            } else if (tol_set_state == TOL_SET_DONE) {
                // 常显2秒
                LCD_Display_Number(temp_tol_standard, dec_places); 
                LCD_Display_Symbol(LCD_SYMBOL_OK, 1);
                LCD_Display_Symbol(LCD_SYMBOL_TOL_LOWER, 1);
                LCD_Display_Symbol(LCD_SYMBOL_TOL_UPPER, 1);
                
                // 判断2秒是否到达
                if (HAL_GetTick() - tol_done_timer > 2000) {
                    app_state = STATE_NORMAL;
                    tol_set_state = TOL_SET_NONE;
                }
            }
...
void App_ProcessKey(KeyMessage_t *key_msg)
{
    ...
    switch (key_msg->code) {
        ...
        case KEY_DOWN_ABS_SET:
            ...
            }else if(app_state == STATE_TOL_SETTING && key_msg->event == KEY_EVENT_SHORT_PRESS){
                // 调整公差：每按一次减一
                if (tol_set_state == TOL_SET_LOWER) temp_tol_lower -= step;
                else if (tol_set_state == TOL_SET_UPPER) temp_tol_upper -= step;
            }
            break;
        ...
        case KEY_TOL:
            ...
            // break;
            if (key_msg->event == KEY_EVENT_LONG_PRESS) {
                if (app_state == STATE_NORMAL) {
                    if (data->tol_enabled) {
                        // 功能5：长按“TOL”进入公差后，再常按“TOL”清公差数据
                        Measure_ClearTolerance();
                    } else {
                        // 功能1：长按“TOL”进入设置，OK闪动，移动使数据为标准值
                        app_state = STATE_TOL_SETTING;
                        tol_set_state = TOL_SET_STANDARD;
                        temp_tol_lower = 0.0f;
                        temp_tol_upper = 0.0f;
                    }
                } else if (app_state == STATE_TOL_SETTING && tol_set_state != TOL_SET_DONE) {
                    // 功能4：长按“TOL” >|OK|<常显2秒，设置完成
                    tol_set_state = TOL_SET_DONE;
                    tol_done_timer = HAL_GetTick();
                    Measure_SetTolerance(temp_tol_standard, temp_tol_upper, temp_tol_lower);
                }
            } else if (key_msg->event == KEY_EVENT_SHORT_PRESS) {
                if (app_state == STATE_TOL_SETTING) {
                    if (tol_set_state == TOL_SET_STANDARD) {
                        // 功能2：短按“TOL”，锁定标准值，>|闪动
                        temp_tol_standard = data->current_value;
                        tol_set_state = TOL_SET_LOWER;
                    } else if (tol_set_state == TOL_SET_LOWER) {
                        // 功能3：短按“TOL”，|<闪动
                        tol_set_state = TOL_SET_UPPER;
                    } else if (tol_set_state == TOL_SET_UPPER) {
                        // 循环回标准值设置（或根据需求做其他处理）
                        tol_set_state = TOL_SET_STANDARD;
                    }
