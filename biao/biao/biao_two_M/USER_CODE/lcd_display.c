...
/*     uint8_t digits[MAX_DIGITS];
   ...
   // 只有当原数为负，且四舍五入后的实际显示值大于0时，才显示负号
   float abs_num = (number < 0) ? -number : number;
   uint32_t total_val = (uint32_t)(abs_num * pow(10, decimal_places) + 0.5f);
   LCD_Display_Symbol(LCD_SYMBOL_NEGATIVE, (number < 0 && total_val > 0) ? 1 : 0);
