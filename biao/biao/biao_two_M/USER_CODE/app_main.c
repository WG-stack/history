...
static EngSettingItem_t current_eng_item = ENG_ITEM_SENSOR_TYPE;
...
static void App_ProcessKey_EngSetting(KeyMessage_t *key);
static void App_EngSetting_UpdateDisplay(void);
...
void App_Init(void)
{
    ...
    // 下载代码后软复位可能导致传感器通信状态异常不发数据，先断电再上电强制复位传感器
    Power_SensorOff();
    HAL_Delay(50);
    ...
    // 检查是否进入工程模式
    if (Key_IsEngineeringModePressed()) {
        app_state = STATE_ENGINEERING_MENU;
        // 初始化工程模式显示
        // 稍后在Run中处理
    }
}
...
void App_Run(void)
{
    ...
    if (has_key_msg) {
        if (app_state != STATE_POWER_OFF &&
            last_key_msg.code == KEY_DOWN_ABS_SET &&
    ...
    switch (app_state) {
        case STATE_POWER_OFF:
            // 待机模式，等待开机键
            if (has_key_msg && last_key_msg.code == KEY_ON_ZERO && last_key_msg.event == KEY_EVENT_LONG_PRESS) {
                Power_On();
                Sensor_GPIO_EnableInterrupt();
                app_state = STATE_NORMAL;
                ...
                LCD_Display_Clear();
                if (Measure_GetData()) {
                    Measure_GetData()->data_updated = 1; // 强制刷新显示
                }
            }
...
//                LCD_Display_Update();
        ...
        case STATE_ENGINEERING_MENU:
            // 工程设置菜单
            if (has_key_msg) {
                App_ProcessKey_EngSetting(&last_key_msg);
            }
            App_EngSetting_UpdateDisplay();
            break;
        ...
        case STATE_TOL_SETTING:
            ...
            // 添加这里的超时判断逻辑：保存成功2秒后返回正常模式
            if (tol_state == TOL_STATE_SAVED && (HAL_GetTick() - tol_save_timer >= 2000)) {
                app_state = STATE_NORMAL;
                // 强制刷新一次正常模式的显示，确保TOL等符号立刻出来
                Measure_GetData()->data_updated = 1; 
            } else {
        ...
        case KEY_M:
            if (key_msg->event == KEY_EVENT_SHORT_PRESS) {
                ...
                if (settings->sensor_type == SENSOR_TYPE_0_0005MM) {
                    ...
                    // 0.001传感器时只有0.001/0.01mm
                    if (res == RESOLUTION_0_001MM) {
                        res = RESOLUTION_0_01MM;
                    } else {
                        res = RESOLUTION_0_001MM;
                    }
                }
...
static void App_ProcessKey_EngSetting(KeyMessage_t *key)
{
    ...
    switch (key->code) {
        case KEY_M:
            if (key->event == KEY_EVENT_SHORT_PRESS) {
                current_eng_item = (EngSettingItem_t)((current_eng_item + 1) % ENG_ITEM_COUNT);
            }
            break;

        case KEY_UP_IN_MM:
            if (key->event == KEY_EVENT_SHORT_PRESS) {
                switch (current_eng_item) {
                    case ENG_ITEM_SENSOR_TYPE:
                        if (settings->sensor_type < SENSOR_TYPE_0_001MM) {
                            settings->sensor_type++;
                        } else {
                            settings->sensor_type = SENSOR_TYPE_0_0005MM;
                        }
                        break;
                    case ENG_ITEM_AVG_TIMES:
                        if (settings->avg_times < AVG_TIMES_16) {
                            settings->avg_times++;
                        } else {
                            settings->avg_times = AVG_TIMES_0;
                        }
                        break;
                    case ENG_ITEM_CLOCK_FREQ:
                        if (settings->sensor_clock < CLK_2MHZ) {
                            settings->sensor_clock++;
                        } else {
                            settings->sensor_clock = CLK_750KHZ;
                        }
                        break;
                    default:
                        break;
                ...
                switch (current_eng_item) {
                    case ENG_ITEM_SENSOR_TYPE:
                        if (settings->sensor_type > SENSOR_TYPE_0_0005MM) {
                            ...
                            settings->sensor_type = SENSOR_TYPE_0_001MM;
                        }
                        break;
                    case ENG_ITEM_AVG_TIMES:
                        if (settings->avg_times > AVG_TIMES_0) {
                            settings->avg_times--;
                        } else {
                            settings->avg_times = AVG_TIMES_16;
                        }
                        break;
                    case ENG_ITEM_CLOCK_FREQ:
                        if (settings->sensor_clock > CLK_750KHZ) {
                            settings->sensor_clock--;
                        } else {
                            settings->sensor_clock = CLK_2MHZ;
                        }
                        break;
                    default:
                        break;
...
static void App_EngSetting_UpdateDisplay(void)
{
    ...
    // 显示 E 表示工程模式
    LCD_Write_Digit(0, 0xE);
    
    // 显示当前设置项的编号 (1~3)
    LCD_Write_Digit(1, current_eng_item + 1);

    switch (current_eng_item) {
        case ENG_ITEM_SENSOR_TYPE:
            // 5 代表 0.0005, 1 代表 0.001
            if (settings->sensor_type == SENSOR_TYPE_0_0005MM) {
                LCD_Write_Digit(4, 5);
            } else {
                LCD_Write_Digit(4, 1);
        ...
        case ENG_ITEM_AVG_TIMES:
            ...
            if (settings->avg_times == AVG_TIMES_0) {
                LCD_Write_Digit(4, 0);
            } else if (settings->avg_times == AVG_TIMES_4) {
                LCD_Write_Digit(4, 4);
            } else if (settings->avg_times == AVG_TIMES_8) {
                LCD_Write_Digit(4, 8);
            } else if (settings->avg_times == AVG_TIMES_16) {
                LCD_Write_Digit(3, 1);
                LCD_Write_Digit(4, 6);
            }
            break;
        ...
        case ENG_ITEM_CLOCK_FREQ:
            ...
            if (settings->sensor_clock == CLK_750KHZ) {
                ...
                LCD_Write_Digit(3, 5);
                LCD_Write_Digit(4, 0);
            } else if (settings->sensor_clock == CLK_1MHZ) {
                LCD_Write_Digit(2, 1);
                LCD_Write_Digit(3, 0);
                LCD_Write_Digit(4, 0);
            } else if (settings->sensor_clock == CLK_1_5MHZ) {
                LCD_Write_Digit(2, 1);
                LCD_Write_Digit(3, 5);
                LCD_Write_Digit(4, 0);
            } else if (settings->sensor_clock == CLK_1_75MHZ) {
                LCD_Write_Digit(2, 1);
                LCD_Write_Digit(3, 7);
                LCD_Write_Digit(4, 5);
            } else if (settings->sensor_clock == CLK_2MHZ) {
                LCD_Write_Digit(2, 2);
                LCD_Write_Digit(3, 0);
                LCD_Write_Digit(4, 0);
            }
