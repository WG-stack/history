...
void MX_TIM7_Init(void)
{
  ...
  // Configure TIM7 for 20Hz update rate (50ms interrupt)
  // 假设APB1时钟为16MHz，预分频为16000-1，则定时器时钟为1kHz
  // 要实现50ms中断，自动重装载值应为50（1kHz * 50ms = 50个计数）
  TIM_InitStruct.Prescaler = 16000 - 1;           // 16MHz / 16000 = 1kHz (1ms per count)
  TIM_InitStruct.CounterMode = LL_TIM_COUNTERMODE_UP;
  TIM_InitStruct.Autoreload = 50 - 1;             // 1kHz * 50ms = 50个计数 (50ms period)
  TIM_InitStruct.ClockDivision = LL_TIM_CLOCKDIVISION_DIV1;
