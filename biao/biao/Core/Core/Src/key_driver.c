...
/* 按键扫描相关定义 */
#define KEY_SHORT_PRESS_MIN_COUNT       2     // 短按最小计数值（2次*50ms = 100ms）
#define KEY_SHORT_PRESS_MAX_COUNT       20    // 短按最大计数值（20次*50ms = 1000ms，超过此值为长按）
#define KEY_LONG_PRESS_MIN_COUNT        20    // 长按检测起始计数值（20次*50ms = 1000ms）

/* 时间阈值定义（单位：毫秒）*/
#define KEY_SCAN_INTERVAL_MS            50    // 50ms扫描周期，与TIM7中断同步
#define KEY_LONG_PRESS_THRESHOLD_MS     (KEY_LONG_PRESS_MIN_COUNT * KEY_SCAN_INTERVAL_MS) // 长按阈值，1000ms

/* 简化定时器按键相关定义 */
#define KEY_SCAN_COUNTS_PER_LONG        KEY_LONG_PRESS_MIN_COUNT  // 达到长按所需的扫描次数
...
// 全局变量声明 - 从global_variables.h引入
extern KeyStateMachine_t key_fsm;                 // 按键状态机变量
extern KeyEventFlags_t key_event_flags;           // 按键事件标志
extern ZTJ_TypeDef measurement_state;             // 测量状态
extern uint8_t LOOP_tol_LCD;                      // 用于公差设置模式的循环计数器

/* 内部按键状态类型定义 */
typedef struct {
    Key_TypeDef key_type;
    uint8_t state;
} InternalKeyState_TypeDef;

/* 内部按键状态 */
static InternalKeyState_TypeDef internal_key_state = {KEY_NONE, 0};  // 使用数字代替枚举常量
static uint32_t key_press_start_interrupt_count = 0;  // 按键按下开始的中断计数

/* 定时器上下文 */
static KeyTimerContext_TypeDef key_timer_context = {0};

// 添加长按状态标志，用于防止按键长按时持续触发功能
static uint8_t m_key_long_press_active = 0;
static uint8_t tol_key_long_press_active = 0;
static uint8_t up_key_long_press_active = 0;
static uint8_t down_key_long_press_active = 0;
static uint8_t on_zero_key_long_press_active = 0;
static uint8_t abs_zero_key_long_press_active = 0;

#define KEY_INTERNAL_STATE_IDLE 0
#define KEY_INTERNAL_STATE_PRESSED 1
#define KEY_INTERNAL_STATE_LONG_PRESS 2
    ...
    switch(internal_key_state.state)
    {
        ...
        case KEY_INTERNAL_STATE_PRESSED:
            if(current_key == internal_key_state.key_type)
            {
                // 按键仍然按下，检查是否达到长按阈值
                // 计算按下时间（单位：ms）
                // 使用预定义的扫描间隔常量，确保与TIM7中断同步
                uint32_t press_time;
                if (current_interrupt_count >= key_press_start_interrupt_count) {
                    // 未溢出情况
                    press_time = (current_interrupt_count - key_press_start_interrupt_count) * KEY_SCAN_INTERVAL_MS;
                } else {
                    // 溢出情况
                    press_time = ((0xFFFFFFFF - key_press_start_interrupt_count) + current_interrupt_count + 1) * KEY_SCAN_INTERVAL_MS;
                }
                
                // 长按阈值设为1000ms
                if(press_time >= KEY_LONG_PRESS_THRESHOLD_MS)  // 使用预定义的长按阈值（1000ms）
                {
                    // 进入长按状态
            ...
            else if(current_key == KEY_NONE)
            {
                ...
                // 处理短按情况（按下时间在100ms至1000ms之间）
                if(press_time >= KEY_SHORT_PRESS_MIN_COUNT * KEY_SCAN_INTERVAL_MS && 
                   press_time < KEY_LONG_PRESS_THRESHOLD_MS)  // 使用常量，避免重复计算
                {
...
__attribute__((used)) void KeyDriver_Process(KeyScanResult_TypeDef *result)
{
   ...
   switch(result->event)
   {
               ...
               switch(result->key_type)
               {
                   ...
                   case KEY_UP_IN_MM:
                       ...
                       if(measurement_state.mode == MEASUREMENT_MODE_TOL_SETTING) {
                           ...
                           // HAL_Delay(100);  // 短暂显示 - 移除阻塞式延时，避免卡在tick
           ...
           switch(result->key_type)
           {
               case KEY_TOL:
                   ...
                   if (!tol_key_long_press_active) {
                       ...
                       // 标记TOL键长按状态为活跃
                       tol_key_long_press_active = 1;
                   }
                   break;
               case KEY_M:
                   // 只在非活跃状态下处理M键长按
                   if (!m_key_long_press_active) {
                       // M键长按处理 - 循环切换分辨率模式 (0.5u, 1u, 0.01mm)
                       ZTJ_CycleResolutionMode();
                       
                       // 标记M键长按状态为活跃
                       m_key_long_press_active = 1;
                   }
                   break;
               case KEY_UP_IN_MM:
                   // 只在非活跃状态下处理UP键长按
                   if (!up_key_long_press_active) {
                       ...
                       // 标记UP键长按状态为活跃
                       up_key_long_press_active = 1;
                   }
                   break;
               case KEY_DOWN_ABS_SET:
                   // 只在非活跃状态下处理DOWN键长按
                   if (!down_key_long_press_active) {
                       ...
                       // 标记DOWN键长按状态为活跃
                       down_key_long_press_active = 1;
                   }
                   break;
               case KEY_ON_ZERO:
                   // 只在非活跃状态下处理ON/ZERO键长按
                   if (!on_zero_key_long_press_active) {
                       // ON/ZERO键长按处理 - 开关机
                       ZTJ_TogglePower(ZTJ_GetCurrentState());
                       
                       // 标记ON/ZERO键长按状态为活跃
                       on_zero_key_long_press_active = 1;
                   }
                   break;
               case KEY_ABS_ZERO_SWITCH:
                   // 只在非活跃状态下处理ABS_ZERO键长按
                   if (!abs_zero_key_long_press_active) {
                       // ABS_ZERO开关长按处理
                       
                       // 标记ABS_ZERO键长按状态为活跃
                       abs_zero_key_long_press_active = 1;
                   }
                   break;
               default:
                   break;
       ...
       case KEY_EVENT_LONG_PRESS_RELEASE:
       case KEY_EVENT_RELEASE:
           // 按键释放处理，包括长按后的释放
           // 重置长按状态标志，允许下次长按时触发
           m_key_long_press_active = 0;
           tol_key_long_press_active = 0;
           up_key_long_press_active = 0;
           down_key_long_press_active = 0;
           on_zero_key_long_press_active = 0;
           abs_zero_key_long_press_active = 0;
           break;
...
void KeyDriver_Timer_Scan(void) {
   ...
   if (current_key_state != 0) {
       ...
       if (prev_key_state == 0) {
           // 新按键按下，记录长按开始时间
           key_timer_context.long_press_start_time = current_interrupt_count;  // 使用中断计数而非HAL_GetTick()
           key_timer_context.long_press_counter = 1;
           
           // 生成按键按下事件，但不立即处理，因为需要等到确定是短按还是长按
       } else {
           // 按键保持按下状态，增加计数器
           // 计算当前按下的持续时间（基于中断计数）
           uint32_t press_duration;
           if (current_interrupt_count >= key_timer_context.long_press_start_time) {
               press_duration = (current_interrupt_count - key_timer_context.long_press_start_time) * KEY_SCAN_INTERVAL_MS;
           } else {
               // 处理中断计数器溢出
               press_duration = ((0xFFFFFFFF - key_timer_context.long_press_start_time) + current_interrupt_count + 1) * KEY_SCAN_INTERVAL_MS;
           }
           
           // 检查是否达到长按阈值（基于中断计数）
           if (press_duration >= KEY_LONG_PRESS_MIN_COUNT * 50) {  // 20 * 50ms = 1000ms
               // 达到长按阈值，发送长按事件
               ...
               // 标记已经处理过长按，防止重复处理
               key_timer_context.long_press_counter = KEY_LONG_PRESS_MIN_COUNT + 1; // 设置一个标记值
           }
       }
       ...
       // 判断按键类型：短按或长按
       if (press_duration >= KEY_SHORT_PRESS_MIN_COUNT * KEY_SCAN_INTERVAL_MS && press_duration < KEY_LONG_PRESS_MIN_COUNT * KEY_SCAN_INTERVAL_MS) {  // 100ms-1000ms之间
           // 短按事件（计数值大于等于2小于20）
           KeyScanResult_TypeDef key_result = {
           ...
           KeyDriver_Process(&key_result);
       } else if (press_duration >= KEY_LONG_PRESS_MIN_COUNT * 50) {  // 大于等于1000ms
           // 长按释放事件（计数值大于等于20）
           KeyScanResult_TypeDef key_result = {
               .key_type = (Key_TypeDef)prev_key_state,
               .event = KEY_EVENT_LONG_PRESS_RELEASE,  // 新增的长按释放事件类型
               .is_pressed = false
           };
       ...
       key_timer_context.long_press_start_time = 0;
   }
   
   /* USER CODE END PFP */
}
//#endif /* ENABLE_KEY_SCAN */


// 在文件末尾添加一个空行，以满足编译器要求
