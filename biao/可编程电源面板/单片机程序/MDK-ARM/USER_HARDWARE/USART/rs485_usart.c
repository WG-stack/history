...
/* RS485串口接收缓冲区定义 */
USART_REC_U4 RS485_2_USART = {0};

/**
 * @brief RS485通讯串口初始化
 * @note UART4用于RS485通讯(固定输出)，使能DMA接收和空闲中断
 */
void RS485_USART_Init(void)
{
    RS485_USART_ClearRxBuffer();
    
    // UART4的DMA接收和空闲中断已在CubeMX中配置
    // 这里可以添加其他初始化代码
}
...
{
    HAL_UART_Transmit_DMA(&huart4, &data, 1);
}
...
void RS485_USART_Parse(void)
{
    ...
    {
        // TODO: 根据具体RS485协议解析接收到的数据
        // 示例：
        // uint8_t *data = RS485_2_USART.USART_DMA_REbuffer;
        // uint16_t len = RS485_2_USART.USART_RE_length;
...
{
    RS485_2_USART.USART_DMA_RX_STA = 0;
    RS485_2_USART.USART_RE_length = 0;
    memset(RS485_2_USART.USART_DMA_REbuffer, 0, sizeof(RS485_2_USART.USART_DMA_REbuffer));
}
...
{
    return RS485_2_USART.USART_RE_length;
}
...
{
    return RS485_2_USART.USART_DMA_REbuffer;
}
...
{
    return (huart4.gState == HAL_UART_STATE_READY) ? 1 : 0;
}
